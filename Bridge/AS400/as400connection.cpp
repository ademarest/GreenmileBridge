...
bool AS400::getRouteDataForGreenmile(const QDate &date, const int chunkSize)
{
    QString queryString =
              "SELECT "
              "TRIM(orderHeaders.HHHRTEN) AS \"route:key\", "
              "DATE(TIMESTAMP_FORMAT(CHAR(orderHeaders.HHHDTEI), \'YYYY-MM-DD\')) AS \"route:date\", "
              "TRIM(orderHeaders.HHHCUSN) AS \"location:key\", "
              "TRIM(orderHeaders.HHHCNMB) AS \"location:description\", "
              "TRIM(orderHeaders.HHHCA1B) AS \"location:addressLine1\", "
              "TRIM(orderHeaders.HHHCA2B) AS \"location:addressLine2\", "
              "TRIM(orderHeaders.HHHCTYB) AS \"location:city\", "
              "TRIM(orderHeaders.HHHSTEB) AS \"location:state\", "
              "TRIM(orderHeaders.HHHZPCB) AS \"location:zipCode\", "
              "orderHeaders.HHHSTPN AS \"stop:baseLineSequenceNumber\", "
              "orderHeaders.HHHORNR AS \"order:number\", "
              "TRIM(companyInfo.L1NAME) AS \"organization:key\", "
              "sizeInfo.\"order:pieces\" AS \"order:pieces\", "
              "sizeInfo.\"order:weight\" AS \"order:weight\", "
              "sizeInfo.\"order:cube\" AS \"order:cube\", "
              ...
              "INNER JOIN(SELECT HHHCUSF AS \"as400:masterStopNumber\", SUM(HHHQYSA) AS \"order:pieces\", SUM(HHHEXSH) AS \"order:weight\", SUM(HHHEXCB) AS \"order:cube\" FROM PWRDTA.HHHORDHL6 WHERE HHHDTEI = " + date.toString("yyyyMMdd") +" GROUP BY HHHCUSF) AS sizeInfo "
              ...
              "orderHeaders.HHHORNR, "
              "companyInfo.L1NAME, "
              "sizeInfo.\"order:pieces\", "
              "sizeInfo.\"order:weight\", "
              "sizeInfo.\"order:cube\", "
