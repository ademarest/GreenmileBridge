...
bool AS400::getRouteDataForGreenmile(const QDate &date, const int chunkSize)
{
    QString queryString =
              ...
              "CASE WHEN TRIM(assignmentInfo.EERDRVN) = '' THEN NULL ELSE TRIM(assignmentInfo.EERDRVN) END AS \"driver:key\", "
              "CASE WHEN TRIM(assignmentInfo.EERTKNB) = '' THEN NULL ELSE TRIM(assignmentInfo.EERTKNB) END AS \"equipment:key\", "
              ...
              "INNER JOIN (SELECT HHHCUSF AS \"location:key\", HHHRTEN AS \"route:key\", HHHINVN AS \"route:date\", MAX(HHHSTPN) AS \"stop:baseLineSequenceNum\" FROM PWRDTA.HHHORDHL6 WHERE HHHDTEI = "+date.toString("yyyyMMdd")+" AND HHHCUSF = HHHCUSN GROUP BY HHHCUSF, HHHRTEN, HHHINVN) AS stopInfo "
              "ON TRIM(stopInfo.\"location:key\") = TRIM(orderInfo.HHHCUSF) AND TRIM(stopInfo.\"route:key\") = TRIM(orderInfo.HHHRTEN) AND TRIM(stopInfo.\"route:date\") = TRIM(orderInfo.HHHINVN)"
